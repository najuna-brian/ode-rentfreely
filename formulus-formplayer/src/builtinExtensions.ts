...
export async function getDynamicChoiceList(
  ...
  try {
    ...
    console.log(
      "[getDynamicChoiceList] WHERE clause from params:",
      whereClause
    );
    // #endregion
    ...
    if (filterParams.length > 0) {
      ...
      if (hasEmptyValue) {
        // #region agent log
        console.log(
          "[getDynamicChoiceList] Has empty filter value, returning empty choices"
        );
        // #endregion
    ...
    if (usesAgeFromDob && whereClause) {
      ...
      // #region agent log
      console.log(
        "[getDynamicChoiceList] Using age_from_dob(), SQL WHERE clause:",
        whereClause
      );
      console.log(
        "[getDynamicChoiceList] Original WHERE clause:",
        originalWhereClause
      );
      // #endregion
    }
    ...
    // #region agent log
    console.log(
      "[getDynamicChoiceList] Got observations:",
      observations.length
    );
    if (observations.length === 0) {
      console.warn(
        "[getDynamicChoiceList] No observations returned from database query"
      );
      console.warn("[getDynamicChoiceList] Query params:", {
        formType: queryName,
        whereClause,
      });
    }
    ...
    if (usesAgeFromDob && originalWhereClause) {
      ...
      if (ageConditions.length > 0) {
        // #region agent log
        console.log(
          "[getDynamicChoiceList] Filtering by calculated age in JavaScript:",
          ageConditions
        );
        console.log(
          "[getDynamicChoiceList] Total observations before age filtering:",
          observations.length
        );
        // #endregion
        ...
        if (observations.length > 0) {
          ...
          console.log("[getDynamicChoiceList] Sample observation:", {
            ...
            sampleDob,
            sampleAge,
            sampleDataKeys: sampleObs.data
              ? Object.keys(sampleObs.data).slice(0, 10)
              : [],
          });
        }
        ...
        try {
          observations = observations.filter((obs: any) => {
            ...
            if (age === null) {
              ...
              if (observations.indexOf(obs) < 3) {
                console.log(
                  "[getDynamicChoiceList] Excluding observation - age is null (no valid dob)"
                );
              }
              // #endregion
              return false;
          ...
          // #region agent log
          console.error(
            "[getDynamicChoiceList] Error during age filtering:",
            filterError
          );
        ...
        // #region agent log
        console.log(
          "[getDynamicChoiceList] After age filtering:",
          observations.length
        );
        if (observations.length === 0) {
          console.warn(
            "[getDynamicChoiceList] No observations passed age filter. Possible issues:"
          );
        ...
        // #region agent log
        console.warn(
          "[getDynamicChoiceList] age_from_dob() detected but no age conditions parsed from:",
          originalWhereClause
        );
        // #endregion
      }
    ...
    if (choices.length === 0) {
      console.warn(
        `getDynamicChoiceList: No valid choices found for query '${queryName}'`
      );
    }
